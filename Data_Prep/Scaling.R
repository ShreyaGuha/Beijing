...
dt.tot <- data.table(PM_12, PM_13, PM_14, PM_15, PM_16, PM_17, PM_18)
...
PM_met <- rbind(c(PM_12, PM_13, PM_14, PM_15, PM_16, PM_17, PM_18))
PM_met <- t(PM_met)
PM_met <- as.data.table(PM_met)
PM_met$date <- seq(ymd('2012-01-01'), ymd('2018-12-31'), by = "1 day")
...
#build a subset with only the variables required
Beijing_plot_seasonal <- subset(Beijing_2011_18, select = c(kz_PM_seasonal, kz_temp_seasonal, kz_prec_seasonal,  
                                                            kz_wind_seasonal, kz_rh_seasonal, kz_pbl_seasonal,                                                   
                                                            year, date))
#use melt function to define the x-axis, against what you're plot_seasonalting
Beijing_plot_seasonal.melt <- melt( Beijing_plot_seasonal, id.vars = c( 'year', 'date'))
Beijing_plot_seasonal.melt$date <- as.Date( Beijing_plot_seasonal.melt$date)

#set background theme
theme_set(theme_bw())
#plot_seasonal
ggplot( Beijing_plot_seasonal.melt) + (aes( x = date, y = value,
                                                     group = variable, alpha = variable)) +
  geom_line() + 
  facet_wrap( . ~ variable)
...
Beijing_plot_STM <- subset(Beijing_2011_18, select = c(PM_STM, temp_STM, prec_STM, 
                                                       ...
                                                       year, date))
#use melt function to define the x-axis, against what you're plot_STMting
...
#build a subset with only the variables required
Beijing_plot_2 <- subset(Beijing_2011_18, select = c(PM2.5_avg_conc, kz_PM_annual, kz_PM_seasonal, PM_STM, 
                                                     year, date))
#use melt function to define the x-axis, against what you're plotting
